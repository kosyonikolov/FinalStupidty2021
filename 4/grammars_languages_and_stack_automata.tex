...
\textit{\textbf{Деф}} - Нека $G = (V, \Sigma, R, S)$ е КСГ и изводите $D$ и $D'$ са такива, че:
\begin{itemize}
...
Преход от тип 1 ще наричаме $((s, \varepsilon, \varepsilon), (q, S))$, от тип 2 - $((q, \varepsilon, A), (q, \gamma))$, за $ A \to_G \gamma \in R$, от тип 3 - $((q, a, a), (q, \varepsilon))$, за $a \in \Sigma$.
...
Ще докажем чрез индукция относно \textbf{броя на преходите от тип 2} в изчислението на автомата $M$, че $S \xRightarrow[]{L}_G^* w\alpha$.
