...
$\Phi(G) = max(\{|\alpha| | A \to_G \alpha \in R, A \in V \backslash \Sigma, \alpha \in V^*\})$.
...
\textit{\textbf{Thm (лема за разрастването за КСЕ)}} - Нека $G = (V, \Sigma, R, S)$ е КСГ. Тогава $\forall w \in L(G): |w| > \Phi(G)^{|V \backslash \Sigma|} \exists$ думи $u, v, x, y, z \in \Sigma*$: $w=uvxyz$, $vy \neq \varepsilon$ и $\forall n \in \mathbb{N}$ е изпълнено, че $uv^nxy^nz \in L(G)$.
