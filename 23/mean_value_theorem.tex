...
\bigbreak
Забелязваме, че $\phi(t) = f(x) - \frac{f^{(0)}(t)}{0!}*(x-t)^0 - \sum_{k=1}^{n}\frac{f^{(k)}(t)}{k!}*(x-t)^k = f(x) - f(t) - \sum_{k=1}^{n}\frac{f^{(k)}(t)}{k!}*(x-t)^k$,
т.е. $\phi(x) = f(x) - f(x) - \sum_{k=1}^{n}\frac{f^{(k)}(x)}{k!}*(x-x)^k = 0$
\bigbreak
Щом $\phi(\xi) = R(x), \phi(x) = 0$, то $R(x) = -\frac{\phi'(c)*(\psi(x)-\psi(\xi))}{\psi'(c)} = \frac{\psi(x)-\psi(\xi)}{\psi'(c)}*\frac{f^{(n+1)}(c)}{n!}*(x-c)^n$\newline
